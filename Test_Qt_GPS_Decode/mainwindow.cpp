    ...
    //读取串口数据
        ...
        for(int i=0;i<readComData.size();i++){
            ...
            if(error == 0)
            {
                ...
                {
                    QString mGGA;
                    mGGA.sprintf("$GGA 有效:%d;时间:分%d秒%d;海拔高%d;水平高:%d",G_GPS_GGA.fix_quality,
                                 G_GPS_GGA.time.minutes,G_GPS_GGA.time.seconds,G_GPS_GGA.altitude,G_GPS_GGA.height);
                ...
                {
                    QString mRMC;
                    mRMC.sprintf("$RMC 有效:%d;时间:月%d日%d;纬度:%d;经度:%d",G_GPS_RMC.valid,
                                 G_GPS_RMC.date.month,G_GPS_RMC.date.day,G_GPS_RMC.latitude.value,G_GPS_RMC.longitude.value);
            ...
            if(error == 2)
            {
                ui->textBrowser_GPSDecode->append("数据解析错误，有可能奇偶检校错误 或 数据判断错误！");
            }
        }
    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(ui->pushButton_Open,&QAbstractButton::clicked,this,&MainWindow::SerialPortOpen);
    ...
    for(int i=0;i<mCharLength;i++){
        ...
        if(error == 0)
        {
            ...
            if(G_GPS_GGA_IsValide == 1)
            {
                QString mGGA;
                mGGA.sprintf("$GGA 有效:%d;时间:分%d秒%d;海拔高%d;水平高:%d",G_GPS_GGA.fix_quality,
                             G_GPS_GGA.time.minutes,G_GPS_GGA.time.seconds,G_GPS_GGA.latitude,G_GPS_GGA.height);
                ui->textBrowser_GPSDecode->append(mGGA);
                G_GPS_GGA_IsValide = 0;
            }
            if(G_GPS_RMC_IsValide == 1)
